...
void 
HNCNCDevice::handleOption( const std::string& name, const std::string& value )
{
...
int 
HNCNCDevice::main( const std::vector<std::string>& args )
{
...
void
HNCNCDevice::generateNewHealthState()
{
...
HNCNCDevice::startAction()
{
    ...
    switch( m_curUserAction->getType() )
    {
        ...
        case HNCNC_ATYPE_GET_SEQ_DEF_INFO:
            if( m_curMachine )
            {
              m_curUserAction->setResponseJSON( m_curMachine->getSequenceDefinitionJsonStr( m_curUserAction->getSeqDefID() ) );
            }
...
HNCNCDevice::dispatchEP( HNodeDevice *parent, HNOperationData *opData )
{
    ...
    else if( "getSeqDefInfo" == opID )
    {
        ...
        std::cout << "=== Get Seq Def Info Request (id: " << seqID << ") ===" << std::endl;
